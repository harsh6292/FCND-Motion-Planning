...
class MotionPlanning(Drone):
    ...
    def plan_path(self):
        ...
        with open("colliders.csv", 'r') as city_data:
            first_line = city_data.readline()
            lat_lon_string = first_line.split(',')

            home_lat = float(lat_lon_string[0].split(' ')[-1])
            home_lon = float(lat_lon_string[1].split(' ')[-1])
            city_data.close()
    ...
    def collinearity_check(self, p1, p2, p3, epsilon=1e-6):
        matrix = np.concatenate((p1, p2, p3), 0)
        determinant = np.linalg.det(matrix)

        return np.abs(determinant) < epsilon
    ...
    def prune_path(self, path):
        ...
        while i < (len(pruned_paths) -2):
            p1 = self.point(pruned_paths[i])
            p2 = self.point(pruned_paths[i+1])
            p3 = self.point(pruned_paths[i+2])

            if (self.collinearity_check(p1, p2, p3)):
                pruned_paths.remove(pruned_paths[i+1])
            else:
                i += 1
